...
/*
  ...
  Names filename are here supposed to be hardcoded, so be sure to rename the input files
  or change the files name in the source code
 
  loadNames("male_firstnames.dat");
  loadNames("female_firstnames.dat");
  loadNames("lastnames.dat");
  ...
  It requires an efficient data structures that provides a very fast lookup.
  Nonetheless I used here a simple std::map with an average insert/lookup complexity of O(logN)
  ...
  (STATE_0, FirstName_found)	->	ACTION: store offset and go to STATE_1 // e.g. "Michelle"+followed by middle names, lastname or any other token
 
  (STATE_1, FirstName_found)	->	ACTION: read next token and stay in STATE_1 // e.g. "Michelle Pamela"
  (STATE_1, LastName_found)	->	ACTION: read next token and go to STATE_0 // e.g. "Michelle Smith" or "Michelle Pamela Smith" (FirstName)+ Lastname
  (STATE_1, Any_found)	->	ACTION: read next token and go to STATE_0 // e.g. "Michelle and .."
 
 
  > Complexity <
 
  Given N the number of names loaded , using a data structure with lookup complexity of O(logN) :
  Names loading: N*O(logN)
  Lookups: O(logN)* #tokens
  Overall complexity: Names loading + Lookups = N*O(logN) + O(logN)* #tokens ~ O(logN)* #tokens   when #token >> N 
